...
BEGIN
  FOR r IN SELECT schemaname, tablename FROM pg_tables WHERE schemaname = 'public' LOOP
    FOR p IN SELECT policyname FROM pg_policies WHERE schemaname = 'public' AND tablename = r.tablename LOOP
      EXECUTE 'DROP POLICY IF EXISTS ' || quote_ident(p.policyname) || ' ON public.' || quote_ident(r.tablename);
    END LOOP;
  END LOOP;
END $$;
...
-- Tabela: volunteer_unavailability
